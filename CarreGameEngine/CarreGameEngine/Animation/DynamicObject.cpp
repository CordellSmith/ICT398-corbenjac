...
void DynamicObject::Transform(md2_model_t * m_Object, const glm::mat4 &TransformationMatrix)
{
	...
	//glm::mat4 FinalTransform = TransformationMatrix * glm::rotate(-90.0f,glm::vec3(1.0f,0.0f,0.0f)) * glm::rotate(-90.0f,glm::vec3(0.0f,0.0f,1.0f));
...
void DynamicObject::Draw()
{
	...
	//TextureManager::Instance()->BindTexture(m_textureId);
...
void  DynamicObject::UpdateVAO()
{
	...
	if ((m_iFrame < 0) || (m_iFrame + 1 > m_finalObject.size() - 1))
	return;
...
void DynamicObject::LoadTexture(md2_model_t * m_Object)
{
	if(m_Object->header.num_skins != 0)
	{
		...
		if (TextureManager::Instance().LoadTexture((m_sFile.substr(0, flo) + "/" + m_Object->skins[0].name).c_str())){
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
		}else{
			//GLubyte data[1];
			//data[0] = 255;
			//TextureManager::Instance()->CreateTexture2D(m_textureId, 1, 1, GL_RGB, GL_RGB, GL_UNSIGNED_BYTE, GL_NEAREST, GL_NEAREST, data);
		}
	}
	else
	{
...
*
* @params vel new velocity
*/
//Method to set velocity of the animation
void DynamicObject::SetVelocity(float vel){
	m_fAnimationVelocity = vel;
}
